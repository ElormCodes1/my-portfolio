...
async function getPostContent(slug: string) {
  const foldertext = "posts";
  const dir = path.resolve("./public", foldertext);
  // const folder = process.env.NODE_ENV === "development" ? "public/posts" : dir;
  const file = `${dir}/${slug}.md`;
  const getcontent = await fs.readFileSync(file, "utf-8");
  // const files = fs.readdirSync(folder);
  // const post = files.find((filename) => filename.replace(".md", "") === slug);
  const { data: frontmatter, content } = matter(getcontent);
  return {
...
async function BlogSidebarPage(props: any) {
                  ...
                  <div className="mr-10 mb-5 flex items-center">
                    <div className="mr-4">
                      <div className="relative h-10 w-10 overflow-hidden rounded-full">
                        <Image
                          src={content.frontmatter.author.image}
                          alt="author"
                          fill
                        />
                      </div>
                    ...
                    <div className="w-full">
                      <h4 className="mb-1 text-base font-medium text-body-color">
                        By <span>{content.frontmatter.author.name}</span>
                      </h4>
                    </div>
                  </div>
